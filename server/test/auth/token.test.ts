...
describe("session token", () => {
  it("verifies a freshly signed token and returns its payload", () => {
    const now = 1_000;
    const token = createSessionToken(SECRET, { ttlMs: 5_000, now, userId: "u1" });

    const payload = verifySessionToken(token, SECRET, { now });

    expect(payload).toMatchObject({ iat: 1_000, exp: 6_000, userId: "u1" });
  });

  it("rejects a token whose userId is not a string", () => {
    const body = Buffer.from(
      JSON.stringify({ iat: 1_000, exp: 9_999_999, userId: 1 }),
    ).toString("base64url");
    const sig = createHmac("sha256", SECRET).update(body).digest("base64url");

    expect(verifySessionToken(`${body}.${sig}`, SECRET, { now: 1_000 })).toBeNull();
  });

  it("rejects a token signed with a different secret", () => {
    const token = createSessionToken(SECRET, { ttlMs: 5_000, now: 1_000, userId: "u1" });
  ...
  it("rejects a token whose payload has been tampered with", () => {
    ...
    const forgedBody = Buffer.from(
      JSON.stringify({ iat: 1_000, exp: 9_999_999, userId: "u1" }),
    ).toString("base64url");
